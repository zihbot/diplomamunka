...
A határokat $\langle e_{1}, e_{2}, \dots, e_{L-1}\rangle$ formában keressük, melynek a diszkrét eloszlását a $\langle p_{1}, p_{2}, \dots, p_{L-1}\rangle$ valószínűségek adják meg, és teljesítik a következő feltételeket \cite{nojavan2017comparative}:
\begin{align}
    p_{1}e_{1} + p_{2}e_{2} + \dots + p_{L-1}e_{L-1} &= q x_{1} + q x_{2} + \dots + q x_{N} = m \\
    p_{1}(e_{1} - m)^{2} + \dots + p_{L-1}(e_{L-1} - m)^{2} &= q(x_{1} - m)^{2} + \dots + q(x_{N} - m)^{2} \\
    p_{1}(e_{1} - m)^{2L-1} + \dots + p_{L-1}(e_{L-1} - m)^{2L-1} &= q(x_{1} - m)^{2L-1} + \dots + q(x_{N} - m)^{2L-1} \\
    p_{1} + p_{2} + \dots + p_{L-1} &= 1 \\
    q &= \frac{1}{N}
\end{align}
ahol \emph{e} a folytonos változó értékei, \emph{N} a számosságuk. Az \emph{m} került bevezetésre az adatsor átlagának jelölésére, a \emph{q} pedig az adatsor normalizálására.

A fenti egyenleteket teljesítő határpontok minden rendbeli nyomatéka a megegyezik az eredeti adatsor adott nyomatékával. A határpontok számának növelésével növekszik az egyenletek száma és komplexitása. Az egyenletrendszert numerikusan is nehéz megoldani, mert nem konvex probléma, így az összes lehetséges értéken kell keresést végezni. A lehetséges értékek pedig a $ \mathbb{R}^{L-1} $ halmazban vannak. Lehetséges csak egy bizonyos pontosságig végezni a keresést és a minimum és maximum közé szorítani, így véges számú értéket kell kipróbálni. Ám az intervallumok számával exponenciálisan növekszik ezek száma, így könnyen végrehajthatatlanná válik.
